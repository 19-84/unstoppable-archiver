...
# Module-private aliases preserved for existing call sites.
_WAYBACK_STRIP_SELECTORS = WAYBACK_STRIP_SELECTORS
_ARCHIVE_TODAY_STRIP_SELECTORS = ARCHIVE_TODAY_STRIP_SELECTORS
...
async def check_archive_today_availability(url: str) -> bool:
    """Thin bool-returning wrapper over `find_archive_today_snapshot`.

    Kept for backwards compatibility with older call sites. New code
    should use `find_archive_today_snapshot` directly — it returns the
    specific memento URL we found (enabling direct-fetch) rather than
    just a yes/no.
    """
    return (await find_archive_today_snapshot(url)) is not None
